...
export const RequireScope = (scope: keyof Permissions) => SetMetadata('scope', scope);

export function HasScope(scope: keyof Permissions) {
  return applyDecorators(HasSession(), SetMetadata('scope', scope), UseGuards(PermissionScopeGuard));
}
