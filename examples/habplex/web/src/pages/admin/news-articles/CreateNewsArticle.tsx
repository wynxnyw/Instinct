...
export function CreateNewsArticle() {
  ...
  async function onSave(article: Article): Promise<void> {
    const newArticle = await articleService.create({ title: article.title, header: article.headerImage, categoryID: 1, shortStory: article.description, image: article.thumbnailImage })
    setLocation(`/admin/news/${newArticle.id!}`);
  }

  return (
    <NewsArticleEditor onSave={onSave}/>
  )
}