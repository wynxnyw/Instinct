...
export function RedeemForgotPassword() {
  const [ state, setState ] = useState<RedeemForgotPasswordState>(defaultForgotPasswordState);

  // eslint-disable-next-line no-restricted-globals
  const params: { token?: string } = parse(location.search);
  const isDisabled = !state.password || !state.passwordAgain;

  function updateField<K extends keyof RedeemForgotPasswordState>(key: K, value: RedeemForgotPasswordState[K]): void {
    setState({
      ...state,
      [key]: value,
    })
            ...
            params.token && (
              <Form className="" onSubmit={onSubmit}>
                ...
                <div className="mt-5">
                  ...
                  <Row>
                    <div className="column-2">
                      <Input type="password" name="password" placeholder="Enter your new password" value={state.password} onChange={updateField}/>
                    </div>
                  </Row>
                </div>
                ...
                <div className="mt-3">
                  ...
                  <Row>
                    <div className="column-2">
                      <Input type="password" name="passwordAgain" placeholder="Enter your new password again" value={state.passwordAgain} onChange={updateField}/>
                    </div>
                  </Row>
                </div>
