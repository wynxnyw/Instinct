      ...
      <Container>
        <Loading isLoading={state.showSpinner}>
          <Column side="left">
            <Card header="Updates">
              {
                state.updates.length === 0 && !state.showSpinner && (
                ...
                <ul>
                  {
                    state.updates.map(update => (
                      <li key={update.id}>
                        ...
                        <p>{update.content}</p>
                        <p style={{ marginTop: -20 }}>Posted by <b>{update.user.username}</b></p>
                      </li>
                    ))
                  }
                </ul>
          ...
          <Column side="right">
            <Card header="Notice">
              ...
              <p>In the event you do encounter a bug, please report it to us <a href="here" target="_blank">here</a>.</p>
              <p>The Instinct development team takes bugs with seriously and aims to fix all bugs within 24 hours.  Bugs will then receive a new test to ensure the same bug never occurs again.</p>
            </Card>
          </Column>
        </Loading>
      </Container>
