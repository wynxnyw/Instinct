...
import {
  ...
  sessionContext,
  themeContext,
  UserContainer, useFetchRoomByID
  ...
  return (
    <UserLayout section="community_rooms">
      <Jumbotron title={room?.name}>
        Owned by <Link to={`/profile/${room?.user?.username}`}>{room?.user?.username}</Link>
      </Jumbotron>
        ...
        <Container>
          <Column side="left">
            <Card header="Room Information">
              <ul>
                <li>Max Users: <b>{room?.maxUsers}</b></li>
                <li>Current Users: <b>{room?.currentUsers}</b></li>
              </ul>
