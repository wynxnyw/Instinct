...
export function FastDataView(buffer, byteOffset, byteLength) {
  this.initCacheArray();
  if (buffer) {
...
FastDataView.prototype.setRange = function (byteOffset, byteLength) {
  ...
  const maxLength = bufferSize - this.byteOffset;
...
FastDataView.prototype.subarray = function (begin, end) {
  ...
  return this.byteArray.subarray(
    ...
    Math.min(this.byteEnd, end + this.byteOffset)
  );
};
...
FastDataView.prototype.pushUint16 = function (value, littleEndian) {
  const offset = this.byteOffset + this.cursor;
  if (littleEndian) {
    this.byteArray[offset + 0] = value;
    this.byteArray[offset + 1] = value >>> 8;
  } else {
